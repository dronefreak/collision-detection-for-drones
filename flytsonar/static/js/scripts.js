...
//                console.log(data.param_info.param_value);
               ...
               case "BAT_N_CELLS":
                   ...
                   break;
                case "PWM_MIN":
                   $("#PWMMin-value").val(parseInt(data.param_info.param_value));
                   $('#PWMMin').spinner({value:parseInt(data.param_info.param_value), step: 5, min: 800, max: 1200});
                   break;
               case "PWM_MAX":
                   $("#PWMMax-value").val(parseInt(data.param_info.param_value));
                   $('#PWMMax').spinner({value:parseInt(data.param_info.param_value), step: 5, min: 1800, max: 2200});
                   break;
               case "PWM_PPM_SEL":
                   if (parseInt(data.param_info.param_value)){
                       $(".rc-ppm").addClass("btn-info");
                       $(".rc-pwm").removeClass("btn-info");
                   }else{
                       $(".rc-pwm").addClass("btn-info");
                       $(".rc-ppm").removeClass("btn-info");
                   }
                    break;
               case "SENS_BOARD_ROT":
                   $("#autopilot_orientation").val(parseInt(data.param_info.param_value));
                   $("#autopilot_orientation").select2();
                   break;
               case "SENS_EXT_MAG_ROT":
                   $("#ext_mag_orientation").val(parseInt(data.param_info.param_value));
                   $("#ext_mag_orientation").select2();
                   break;
               case "RC_MAP_MODE_SW":
                   $("#rc-manual-select").val(parseInt(data.param_info.param_value));
                   $("#rc-manual-select").select2();
                   $("#rc-altcntrl-select").val(parseInt(data.param_info.param_value));
                   $("#rc-altcntrl-select").select2();
//                   $("#poscntrl-select").val(parseInt(data.param_info.param_value));
//                   $("#poscntrl-select").select2();
                   break;
               case "RC_MAP_POSCTL_SW":
                   $("#rc-poscntrl-select").val(parseInt(data.param_info.param_value));
                   $("#rc-poscntrl-select").select2();
                   break;
                case "RC_MAP_OFFB_SW":
                    $("#rc-select").val(parseInt(data.param_info.param_value));
                    $("#rc-select").select2();
                    $("#api-select").val(parseInt(data.param_info.param_value));
                    $("#api-select").select2();
                    break;
                case "RC_MAP_RETURN_SW":
                    $("#rtl-select").val(parseInt(data.param_info.param_value));
                    $("#rtl-select").select2();
                    break;
                case "RC_OFFB_TH":
                    rcOffboardTh=round(data.param_info.param_value,2);
                    $('#api-mode').ionRangeSlider("update", {
                        from: round(data.param_info.param_value,2),

                      });
                    break;
                case "RC_RETURN_TH":
                    rcRtlOnTh=round(data.param_info.param_value,2);
                    $('#rtlon-mode').ionRangeSlider("update", {
                        from: round(data.param_info.param_value,2),

                      });
                    break;
                case "RC_POSCTL_TH":
                    rcPosCtlTh=round(data.param_info.param_value,2);
                    $('#rc-poscntrl-mode').ionRangeSlider("update", {
                        from: round(data.param_info.param_value,2),

                      });
                    break;
                case "RC_ASSIST_TH":
                    rcAltCtlTh=round(data.param_info.param_value,2);
                    $('#rc-altcntrl-mode').ionRangeSlider("update", {
                        from: round(data.param_info.param_value,2),
               ...
               case "ob_track_follow":
                  ...
                  if(parseInt(data.param_info.param_value)) 
                   ...
                   break;
               case "ob_track_kp":
                  $("#ob_track_kp").val(data.param_info.param_value);
                  break;
               case "ob_track_kd":
                  $("#ob_track_kd").val(data.param_info.param_value);
                  break;
               case "ob_track_vel_xy_min":
                  $("#ob_track_vel_xy_min").val(data.param_info.param_value);
                  break;
               case "ob_track_vel_xy_max":
                  $("#ob_track_vel_xy_max").val(data.param_info.param_value);
                  break;
               case "ob_track_mode":
                   // console.log(data.param_info.param_value);
